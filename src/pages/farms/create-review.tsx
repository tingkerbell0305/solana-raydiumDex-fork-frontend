...
export default function CreateFarmReviewPage() {
  ...
  const createFarmButton = (
    <Button
      ...
      size={isMobile ? 'sm' : 'lg'}
      validators={[{ should: haveOver300Ray, fallbackProps: { children: 'Insufficient MAKI balance' } }]}
      onClick={async () => {
        txCreateNewFarm({
  ...
  const estimatedIndicator = (
    <Col className="mt-4 text-sm font-medium items-center">
      <div>
        <span className="text-[#abc4ff80]">Fee:</span> <span className="text-[#abc4ff]">300 MAKI</span>
      </div>
      <div>
  ...
  return (
    <PageLayout metaTitle="Farms - MakiDEX" mobileBarTitle="Create Farm">
      <div className="self-center w-[min(720px,90vw)] py-4">
        ...
        <div className="font-medium text-sm mobile:text-xs text-justify leading-snug text-[#abc4ff80] mb-8">
          ...
          farming starts. Newly created farms generally appear on MakiDEX 30 seconds after creation, depending on
          Solana network status. A one-time fee of 300 MAKI is required to create a farm, which will be deposited into
          the MakiDEX treasury.
        </div>
        ...
        {created ? (
          ...
          <div className="text-[#DA2EEF] font-medium text-center text-sm my-4 mobile:mx-4">
            Creating a farm requires a one-time 300 MAKI fee. Your MAKI balance: {toString(userRayBalance) || 0} MAKI
          </div>
        )}
