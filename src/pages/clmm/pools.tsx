...
export default function PoolsConcentratedPage() {
  ...
  return (
    <PageLayout
      ...
      metaTitle="Concentrated Pools - MakiDEX"
    >
      <PoolHeader />
      <PoolCard />
      <MyPositionDialog />
      ...
      <Col>
        ...
        <div className="font-medium text-[rgba(196,214,255,.5)] text-base mobile:text-sm">
          ...
          <Link
            ...
            href="https://makiso.gitbook.io/maki"
          >
            Learn more
          </Link>
        </div>
...
function PoolCardDatabaseBodyCollapseItemFace({
  ...
  const rewardsBadge = useMemo(() => {
    const badges = info.rewardInfos
      .filter((reward) => reward.endTime > new Date().getTime() - 10 * 24 * 3600 * 1000)
      .map((reward, idx) => {
                ...
                <Row className="text-sm justify-between items-center min-w-[260px] gap-4">
                  ...
                  {isRewardEnd ? null : (
                    <span className="text-white/50">
                      {toUsdVolume(
                        toTotalPrice(
                          reward.rewardPerWeek,
                          variousPrices[toPubString(reward.rewardToken?.mint)] ?? null
                        ),
                        { decimalPlace: 0 }
                      )}
                    </span>
