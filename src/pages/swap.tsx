...
export default function Swap() {
  return (
    <SwapUIContextProvider>
      <SwapEffect />
      <PageLayout mobileBarTitle="Swap" metaTitle="Swap - MakiDEX">
        <SwapHead />
        <SwapCard />
        {/* <UnwrapWSOL /> */}
        <KLineChart />
...
function SwapCardInfo({
  ...
  const routeToken = getToken(currentCalcResult?.routeType === 'route' ? currentCalcResult.middleToken.mint : undefined)
    ...
    >
      ...
      {maxSpent ? (
        <SwapCardItem
          ...
          fieldValue={`${toString(maxSpent, { decimalLength: `auto ${upCoin?.decimals}` })} ${upCoin?.symbol ?? '--'}`}
          tooltipContent="The max amount of tokens you will spend on this trade"
        />
      ) : (
        <SwapCardItem
          fieldName="Minimum Received"
          fieldValue={`${toString(minReceived, { decimalLength: `auto ${downCoin?.decimals}` })} ${
            downCoin?.symbol ?? '--'
          }`}
      ...
      <Collapse openDirection="upwards" className="w-full">
        <Collapse.Body>
          <Col className="gap-3 pb-3">
            ...
            <SwapCardItem
              fieldName="Swap Fee"
              tooltipContent={`Of the 0.25% swap fee, 0.22% goes to LPs and 0.03% is used to buy back MAKI.${
