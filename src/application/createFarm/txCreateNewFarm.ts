...
import { ACTIVE_NETWORK, NETWORK } from '@raydium-io/raydium-sdk'
...
export default async function txCreateNewFarm({
  ...
  return txHandler(async ({ transactionCollector, baseUtils: { owner, connection } }) => {
    ...
    // H2W : MAKI Token Infos
    let lockMint = ""; // Maki Token Mint Pubkey
    let lockVault = ""; // Vault for Farm Create Fee (300 MakiToken), Owner's MakiToken ATA
    if (ACTIVE_NETWORK == NETWORK.MAINNET) {
      lockMint = 'CWzVGw8MjSaJ3mKE4aYNUHDQ8xh3ZCkW5PkdGirZbZAy', // NOTE: Force at 2022-07-06, haven't tell why, just force it
      lockVault = '9UGdemhZdACRK5q2zicVKfmuigDYZsRh8ngaHisFj49a' // NOTE: Force at 2022-07-06, haven't tell why, just force it
    } else {
      lockMint = '7qdPr5uE71pBkdNsSo49mc5iFYJQLQVJjd6TKRikBrk6', // NOTE: Force at 2022-07-06, haven't tell why, just force it
      lockVault = '834auQxpQihH9KcWCtZhU76ggDpTN8popnEikBswwBnq' // NOTE: Force at 2022-07-06, haven't tell why, just force it
    }
