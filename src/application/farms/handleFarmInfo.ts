...
function calculateFarmPoolAprs(
  ...
  if (info.version === 6) {
    return info.state.rewardInfos.map(({ rewardPerSecond, rewardOpenTime, rewardEndTime }, idx) => {
      ...
      const isRewardBeforeStart = isDateBefore(payload.currentBlockChainDate, rewardOpenTime.toString(), { unit: 's' })
      const isRewardAfterEnd = isDateAfter(payload.currentBlockChainDate, rewardEndTime.toString(), { unit: 's' })
...
function whetherIsStakeFarmPool(info: SdkParsedFarmInfo): boolean {
  // H2W : update Staking Pool condition
  // return info.state.rewardInfos.length === 1 && String(info.lpMint) === toPubString(RAYMint) // Ray Mint
  return String(info.lpMint) === toPubString(RAYMint) // Ray Mint
}
